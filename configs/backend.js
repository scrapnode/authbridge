...
function main(templatefile = "./template.json") {
  ...
  fs.writeFileSync(backendfile, JSON.stringify(configs, null, 2));

  const backendmapfile = path.resolve(__dirname, "../backend/configs.map.json");
  fs.writeFileSync(backendmapfile, JSON.stringify(props, null, 2));
}
